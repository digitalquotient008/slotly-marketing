...
export default function CTA({ 
  ...
  return (
    <section className={`py-16 sm:py-20 ${variant === 'primary' ? 'bg-gradient-to-br from-blue-600 to-indigo-700' : 'bg-white'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className={`text-3xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-6 ${variant === 'primary' ? 'text-white' : 'text-gray-900'}`}>
          {title}
        </h2>
        <p className={`text-lg sm:text-xl md:text-2xl mb-8 sm:mb-10 leading-relaxed ${variant === 'primary' ? 'text-blue-100' : 'text-gray-600'}`}>
          {description}
        </p>
        <Link
